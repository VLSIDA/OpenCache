...
class memory_block_fifo(memory_block_base):
    ...
    def add_compare(self, c, m):
        ...
        with m.Case(state.COMPARE):
            ...
            with c.check_dirty_miss(m, c.use_array.output()):
                # If DRAM is available, switch to WAIT_WRITE and wait for DRAM to
                # complete writing
                with m.If(~c.dram.stall()):
                    with m.Switch(c.use_array.output()):
                        for i in range(c.num_ways):
                            with m.Case(i):
                                c.dram.write(Cat(c.set, c.tag_array.output().tag(c.use_array.output())), c.data_array.output(i))
