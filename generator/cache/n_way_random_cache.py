...
class n_way_random_cache(cache_base):
        ...
        with m.Else():
            with m.Switch(self.state):
                ...
                with m.Case(State.COMPARE):
                    ...
                    with m.If(
                        (self.bypass & (self.new_tag.bit_select(self.random * (self.tag_size + 2) + self.tag_size, 2) == Const(3, 2))) |
                        (~self.bypass & (self.tag_read_dout.bit_select(self.random * (self.tag_size + 2) + self.tag_size, 2) == Const(3, 2)))
                    ):
                        ...
                        with m.Else():
                            ...
                            with m.If(self.bypass):
                                m.d.comb += self.main_addr.eq(Cat(self.set, self.new_tag.bit_select(self.random * (self.tag_size + 2), self.tag_size)))
                                m.d.comb += self.main_din.eq(self.new_data.word_select(self.random, self.line_size))
                            with m.Else():
                    ...
                    for i in range(self.num_ways):
                        with m.If(
                            (self.bypass & ~self.new_tag[i * (self.tag_size + 2) + self.tag_size + 1]) |
                            (~self.bypass & ~self.tag_read_dout[i * (self.tag_size + 2) + self.tag_size + 1])
                        ):
                            with m.If(~self.main_stall):
                                m.d.comb += self.tag_read_addr.eq(self.set)
                                m.d.comb += self.data_read_addr.eq(self.set)
    ...
    def add_bypass_block(self, m):
        ...
        # In this block, bypass registers are controlled. Bypass registers
        # are used to prevent data hazard from SRAMs. Data hazard can occur
        # when there are read and write requests to the same row at the same
        # cycle.
