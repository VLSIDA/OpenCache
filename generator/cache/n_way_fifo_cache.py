...
class n_way_fifo_cache(cache_base):
    ...
    def write_registers(self):
        ...
        if self.data_hazard:
            ...
            self.vf.write("  reg [WAY_WIDTH-1:0]                   new_fifo, new_fifo_next; // new replacement bits from the previous cycle\n")
        ...
        self.vf.write("  reg  fifo_read_csb;  // fifo read port active low chip select\n")
        self.vf.write("  reg  [SET_WIDTH-1:0] fifo_read_addr;\n")
        self.vf.write("  wire [WAY_WIDTH-1:0] fifo_read_dout;\n")
        self.vf.write("  reg  fifo_write_csb; // fifo write port active low chip select\n")
